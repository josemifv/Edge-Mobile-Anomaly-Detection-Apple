#!/usr/bin/env python3
"""
Stage 04: OSP (Orthogonal Subspace Projection) Anomaly Detection

This script implements OSP-based anomaly detection for telecommunications data using
per-cell SVD decomposition. It leverages the reference weeks selected in Stage 03
to build normal behavior models for each cell and detect anomalies.

Key Features:
- Per-cell OSP modeling using SVD decomposition
- Configurable subspace dimensions and thresholds
- Apple Silicon optimization (MPS backend support)
- Comprehensive performance benchmarking
- Parallel processing for multiple cells
- Detailed anomaly scoring and reporting
...
class OSPAnomalyDetector:
    """
    Orthogonal Subspace Projection (OSP) Anomaly Detector.
    
    This class implements OSP-based anomaly detection using SVD decomposition
    to model normal behavior patterns for telecommunications data.
    """
    ...
    def get_model_info(self) -> Dict:
        ...
        return {
            ...
            "total_explained_variance": np.sum(self.svd.explained_variance_ratio_),
            "training_errors_mean": np.mean(self.reconstruction_errors),
            "training_errors_std": np.std(self.reconstruction_errors)
        }
...
def process_single_cell(args: Tuple) -> Dict:
    ...
    try:
        ...
        features = cell_data[FEATURE_COLUMNS].values
        ...
        detector = OSPAnomalyDetector(**adjusted_params)
...
def load_data(data_path: Path, reference_weeks_path: Path) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    Load the main dataset and reference weeks.
...
def prepare_cell_data(main_data: pd.DataFrame, 
                     max_cells: Optional[int] = None,
                     min_samples_per_cell: int = 100) -> Dict[int, pd.DataFrame]:
    """
    Prepare per-cell data dictionaries.
...
def run_anomaly_detection(cell_data_dict: Dict[int, pd.DataFrame],
                         reference_weeks: pd.DataFrame,
                         detector_params: Dict,
                         max_workers: int = None,
                         progress_interval: int = 100) -> List[Dict]:
    """
    Run anomaly detection across all cells.
    ...
    print(f"Starting anomaly detection with {max_workers} workers...")
    ...
    if max_workers == 1:
        ...
        for i, args in enumerate(args_list):
            ...
            result = process_single_cell(args)
            results.append(result)
    else:
        # Multi-threaded processing
        with Pool(processes=max_workers) as pool:
            for i, result in enumerate(pool.imap(process_single_cell, args_list)):
                if i % progress_interval == 0:
...
def generate_performance_report(results: List[Dict], 
    ...
    report_file = output_dir / f"osp_anomaly_detection_report_{timestamp}.json"
...
def save_results(results: List[Dict], output_dir: Path, format: str = 'parquet') -> Path:
    ...
    summary_file = output_dir / f"osp_anomaly_summary_{timestamp}.{format}"
    ...
    detailed_file = output_dir / f"osp_anomaly_detailed_{timestamp}.{format}"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="OSP Anomaly Detection for Telecommunications Data",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Basic usage
  uv run python scripts/04_anomaly_detection_osp.py data/processed/consolidated_milan_telecom_merged.parquet reports/reference_weeks_20250607_135521.parquet
  
  # Custom parameters
  uv run python scripts/04_anomaly_detection_osp.py data/processed/consolidated_milan_telecom_merged.parquet reports/reference_weeks_20250607_135521.parquet --n_components 10 --anomaly_threshold 2.5 --max_workers 8
  
  # Test mode with limited cells
  uv run python scripts/04_anomaly_detection_osp.py data/processed/consolidated_milan_telecom_merged.parquet reports/reference_weeks_20250607_135521.parquet --max_cells 100 --output_dir test_results/
        """
    )
    ...
    print("=" * 80)
    print("OSP ANOMALY DETECTION - STAGE 04")
    print("=" * 80)
    ...
    try:
        # Load data
        print("\n1. Loading data...")
        main_data, reference_weeks = load_data(data_path, reference_weeks_path)
        ...
        # Prepare cell data
        print("\n2. Preparing cell data...")
        cell_data_dict = prepare_cell_data(
            main_data, 
        ...
        print("\n3. Running OSP anomaly detection...")
        ...
        print(f"Throughput: {perf_metrics['throughput_samples_per_second']:,.0f} samples/sec, "
              f"{perf_metrics['throughput_cells_per_second']:.1f} cells/sec")
